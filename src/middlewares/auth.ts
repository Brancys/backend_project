...
export function onlyAdminMiddleware(requireSelfOrAdmin: boolean = false) {
  return (request: Request, response: Response, next: NextFunction) => {
      const authHeader = request.headers['authorization'];
      const token = authHeader && authHeader.split(' ')[1];
      ...
      try {
          ...
          if (requireSelfOrAdmin) {
              ...
              if (!isAdmin) {
                  return response.status(403).json({ message: 'Access denied. Only admins can perform this action.' });
              }
          }
